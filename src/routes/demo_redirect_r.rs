use crate::handlers::demo_redirect_h;
use warp::Filter;
...
pub fn index() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
        ...
        .and(warp::path::end())
        .and_then(demo_redirect_h::index)
        .or(v())
        .or(redirect())
        .or(found())
        .or(see_other())
...
pub fn redirect() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
        ...
        .and(warp::path("v1"))
        .and(warp::path::end())
        .map(|| warp::redirect(warp::http::Uri::from_static("/demo/redirect/v")))
}
...
pub fn permanent() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
        ...
        .and(warp::path("v8"))
        .and(warp::path::end())
        .map(|| warp::redirect::permanent(warp::http::Uri::from_static("/demo/redirect/v")))
}
...
pub fn found() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
        ...
        .and(warp::path("v2"))
        .and(warp::path::end())
        .map(|| warp::redirect::found(warp::http::Uri::from_static("/demo/redirect/v")))
}
...
pub fn see_other() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
        ...
        .and(warp::path("v3"))
        .and(warp::path::end())
        .map(|| warp::redirect::see_other(warp::http::Uri::from_static("/demo/redirect/v")))
}
...
pub fn temporary() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
        ...
        .and(warp::path("v7"))
        .and(warp::path::end())
        .map(|| warp::redirect::temporary(warp::http::Uri::from_static("/demo/redirect/v")))
}
...
pub fn post_repeat() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::get()
        ...
        .and(warp::path::param())
        .and(warp::path::end())
        .and_then(demo_redirect_h::repeat)
}
